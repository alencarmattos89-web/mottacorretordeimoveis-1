...
export default function ImovelPage() {
  ...
  async function handleSubmit(e: any) {
    e.preventDefault()
    setEnviando(true)
    await supabase.from('leads').insert({ nome: form.nome, telefone: form.telefone, imovel_id: imovel.id, imovel_titulo: imovel.titulo })
    setEnviado(true)
    setEnviando(false)
  }
  ...
  return (
    <main style={{ background: '#0a0a0a', minHeight: '100vh', fontFamily: 'system-ui,sans-serif' }}>
      <header style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(10,10,10,0.85)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(201,168,76,0.25)', padding: '0 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '72px' }}>
        <Link href="/"><img src="/LOGO_transparente_final.png" alt="Motta Corretor" style={{ height: '52px', objectFit: 'contain' }} /></Link>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '56px 32px', display: 'grid', gridTemplateColumns: '1fr 360px', gap: '48px', alignItems: 'start' }}>
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '32px', flexWrap: 'wrap', gap: '12px' }}>
            <p style={{ fontFamily: 'Georgia,serif', fontSize: '32px', color: '#c9a84c', fontWeight: 400 }}>
        ...
        <div style={{ position: 'sticky', top: '96px' }}>
          <a href={mensagemWhatsApp()} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', background: '#25D366', color: '#fff', textAlign: 'center', padding: '18px', fontSize: '13px', letterSpacing: '2px', textTransform: 'uppercase', fontWeight: 600, textDecoration: 'none', marginBottom: '1px' }}>
            <IconeWhatsApp />
            Falar no WhatsApp
          </a>
              ...
              <form onSubmit={handleSubmit}>
                {[
                  ...
                  <div key={name} style={{ marginBottom: '16px' }}>
                    ...
                    <input type={type} required={required} value={(form as any)[name]} onChange={e => setForm(f => ({ ...f, [name]: e.target.value }))} style={{ width: '100%', background: '#1a1814', border: '1px solid rgba(201,168,76,0.2)', color: '#e8e0d0', padding: '10px 14px', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                ))}
