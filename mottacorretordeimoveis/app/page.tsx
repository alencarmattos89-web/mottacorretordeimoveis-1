  ...
  return (
    <main style={{ background: '#0a0a0a', minHeight: '100vh', fontFamily: 'system-ui,sans-serif' }}>
      ...
      <style>{`
        ...
        .home-hero { min-height: 430px; padding: 100px 32px 80px; display: flex; align-items: center; justify-content: center; position: relative; overflow: hidden; text-align: center; border-bottom: 1px solid rgba(201,168,76,0.15); background: #0f0e0c; }
        .home-hero-img { position: absolute; inset: 0; width: 100%; height: 100%; }
        .home-hero-overlay { position: absolute; inset: 0; background: rgba(10,10,10,0.58); }
        ...
        @media (max-width: 768px) {
          ...
          .home-hero { min-height: 360px !important; padding: 80px 18px 64px !important; }
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(10,10,10,0.92)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(201,168,76,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '72px' }} className="home-header">
        <img src="https://wabkkqbgfwufmxjutxsr.supabase.co/storage/v1/object/public/assets/LOGO%20MOTTA%20site.png" alt="Motta Corretor" className="home-header-logo" />
        <nav className="home-nav-links">
          ...
          <a href="#contato" style={{ color: '#0a0a0a', background: '#c9a84c', padding: '10px 18px', fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', textDecoration: 'none', fontWeight: 700 }}>Contato</a>
        </nav>
      </header>
      ...
      <section className="home-hero">
        ...
        <div className="home-hero-content">
          ...
          <h1 style={{ fontFamily: 'Georgia,serif', fontSize: `clamp(30px, 5vw, ${numero(cfg.hero_titulo_tamanho, 52)}px)`, fontWeight: 300, color: cfg.hero_titulo_cor, lineHeight: 1.2, marginBottom: '12px' }}>{cfg.hero_titulo}</h1>
      ...
      <section id="imoveis" className="imoveis-section" style={{ maxWidth: '1200px', margin: '0 auto', padding: '64px 32px' }}>
        <style>{`
          ...
          @media (max-width: 768px) { .busca-form { grid-template-columns: 1fr 1fr; } .busca-select:first-child { border-bottom: 1px solid rgba(201,168,76,0.15); } .busca-select:nth-child(2) { border-right: none; border-bottom: 1px solid rgba(201,168,76,0.15); } .busca-input { grid-column: 1/-1; border-right: none; border-bottom: 1px solid rgba(201,168,76,0.15); } .busca-btn { grid-column: 1/-1; padding: 16px; } }
        `}</style>
        ...
        {temFiltro && (
          <div style={{ marginTop: '-28px', marginBottom: '40px', display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
            {filtroTipo && <span style={{ fontSize: '11px', padding: '4px 12px', background: 'rgba(201,168,76,0.12)', color: '#c9a84c', border: '1px solid rgba(201,168,76,0.25)', letterSpacing: '1px' }}>{filtroTipo === 'venda' ? 'Comprar' : 'Alugar'}<Link href={urlRemoveFiltro('tipo')} style={{ marginLeft: '8px', color: '#6b6355', textDecoration: 'none' }}>✕</Link></span>}
            {filtroCategoria && <span style={{ fontSize: '11px', padding: '4px 12px', background: 'rgba(201,168,76,0.12)', color: '#c9a84c', border: '1px solid rgba(201,168,76,0.25)', letterSpacing: '1px' }}>{filtroCategoria.charAt(0).toUpperCase() + filtroCategoria.slice(1)}<Link href={urlRemoveFiltro('categoria')} style={{ marginLeft: '8px', color: '#6b6355', textDecoration: 'none' }}>✕</Link></span>}
            {filtroPrecoMax && <span style={{ fontSize: '11px', padding: '4px 12px', background: 'rgba(201,168,76,0.12)', color: '#c9a84c', border: '1px solid rgba(201,168,76,0.25)', letterSpacing: '1px' }}>Até R$ {Number(filtroPrecoMax).toLocaleString('pt-BR')}<Link href={urlRemoveFiltro('preco_max')} style={{ marginLeft: '8px', color: '#6b6355', textDecoration: 'none' }}>✕</Link></span>}
            {filtroBusca && <span style={{ fontSize: '11px', padding: '4px 12px', background: 'rgba(201,168,76,0.12)', color: '#c9a84c', border: '1px solid rgba(201,168,76,0.25)', letterSpacing: '1px' }}>"{filtroBusca}"<Link href={urlRemoveFiltro('busca')} style={{ marginLeft: '8px', color: '#6b6355', textDecoration: 'none' }}>✕</Link></span>}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px', gap: '16px', flexWrap: 'wrap' }}>
          ...
          {imoveis && imoveis.length > 0 && <p style={{ fontSize: '11px', color: '#4a4438', letterSpacing: '1px' }}>{imoveis.length} encontrado{imoveis.length !== 1 ? 's' : ''}{temFiltro && <span style={{ color: '#c9a84c' }}> · filtrado</span>}</p>}
        </div>
        ...
        {imoveis && imoveis.length > 0 ? (
          ...
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            {temFiltro && <Link href="/#imoveis" style={{ color: '#c9a84c', fontSize: '12px', letterSpacing: '2px', textDecoration: 'none' }}>Limpar filtros →</Link>}
          </div>
        )}
      </section>
