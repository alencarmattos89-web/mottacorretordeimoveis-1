#!/bin/bash
# Fix: exclusão funcionando + logo em todas as páginas públicas
set -e
...
LOGO_URL="https://wabkkqbgfwufmxjutxsr.supabase.co/storage/v1/object/public/assets/LOGO%20MOTTA%20site.png"

# ════════════════════════════════════════════════════════════════════════════
# ARQUIVO 1 — app/admin/imoveis/page.tsx
# Exclusão com modal "EXCLUIR" — versão corrigida (delete funcionando)
# ════════════════════════════════════════════════════════════════════════════
cat > "$ROOT/app/admin/imoveis/page.tsx" << 'EOF'
'use client'
...
EOF
echo "✅  page.tsx (listagem) atualizado"

# ════════════════════════════════════════════════════════════════════════════
# ARQUIVO 2 — app/imovel/[id]/ImovelClient.tsx
# Troca /LOGO_transparente_final.png pela URL do Supabase
# ════════════════════════════════════════════════════════════════════════════
FILE_IMOVEL="$ROOT/app/imovel/[id]/ImovelClient.tsx"

# Substitui a logo local pela URL do Supabase no header e no footer
sed -i \
  's|src="/LOGO_transparente_final.png"|src="https://wabkkqbgfwufmxjutxsr.supabase.co/storage/v1/object/public/assets/LOGO%20MOTTA%20site.png"|g' \
  "$FILE_IMOVEL"

echo "✅  ImovelClient.tsx — logo atualizada"
...
git add \
  ...
  "mottacorretordeimoveis/app/imovel/[id]/ImovelClient.tsx"

git commit -m "fix: exclusão com delete real no modal EXCLUIR + logo Supabase em todas as páginas públicas"

git push

echo ""
echo "🚀  Pronto! Commit e push feitos."
